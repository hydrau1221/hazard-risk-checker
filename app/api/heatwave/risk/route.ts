// app/api/heatwave/risk/route.ts
import { NextRequest } from "next/server";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** NRI – mêmes couches publiques que Landslide/Hurricane */
const NRI_TRACTS =
...
/** Label officiel NRI → nos niveaux (+ Not Applicable).
 *  Pour Heatwave, on lit **HWAV_RISKR** (rating) et **HWAV_RISKS** (score 0–100). */
...
/** Optionnel: classification par score (si ?mode=score) */
function mapScoreToLevel(scoreRaw: unknown): Level {
  ...
  // Bornes génériques (cohérentes avec nos autres cartes)
...
/** Extraction stricte des champs Heatwave : HWAV_RISKR / HWAV_RISKS */
function extract(attrs: Record<string, any>) {
  const rate = findAttr(attrs, [/(^|_)HWAV_RISKR$/i, /(^|_)HWAV.*_RISKR$/i]);
  let score = findAttr(attrs, [/(^|_)HWAV_RISKS$/i]);

  if (!score) {
    // fallback prudent : ...HWAV...RISKS (sans RANK/PCTL/INDEX/RISKR)
    const k = Object.keys(attrs).find(k => {
      const up = k.toUpperCase();
      return up.includes("HWAV") && up.endsWith("RISKS")
...
/** Petit buffer géodésique (~50m) pour le dernier recours */
