...
function extract(attrs: Record<string, any>) {
  const isColdKey = (up: string) => (up.includes("CWAV") || up.includes("COLD"));
  const ends = (s: string) => (up: string) => up.endsWith(s);

  const riskR = findAttr(attrs, [ (up) => isColdKey(up) && ends("_RISKR")(up) ]);
  let riskS   = findAttr(attrs, [ (up) => isColdKey(up) && ends("_RISKS")(up) ]);

  if (!riskS) {
    const cand = Object.keys(attrs).find(k => {
      const up = k.toUpperCase();
      return isColdKey(up) && up.endsWith("RISKS") &&
