// app/api/coldwave/risk/route.ts
import { NextRequest } from "next/server";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** NRI – mêmes couches publiques que Landslide/Heatwave/Hurricane */
const NRI_TRACTS =
...
/** Mappe le libellé NRI (CWAV_RISKR) → nos niveaux (+ Not Applicable) */
...
/** Optionnel: classification par score (si ?mode=score) — champs CWAV_RISKS (0–100) */
...
/** Extraction stricte des champs Cold Wave : CWAV_RISKR / CWAV_RISKS */
function extract(attrs: Record<string, any>) {
  const rate = findAttr(attrs, [/(^|_)CWAV_RISKR$/i, /(^|_)CWAV.*_RISKR$/i]);
  let score = findAttr(attrs, [/(^|_)CWAV_RISKS$/i]);

  if (!score) {
    // fallback prudent : ...CWAV...RISKS (sans RANK/PCTL/INDEX/RISKR)
